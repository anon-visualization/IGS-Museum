// temp control of animation, show gallery spaces, video ALL if in zoomview.
function keyPressed() {
	if (zoomView) {
		if (key == 'A' || key == 'a') {
			animate = !animate;
...
function mousePressed() {
	...
	if (zoomView) {
		...
		overZoomSpaceButton();
		overZoomExitButton();
		//make this better/tighter
		if (showing && mouseX > timelineStart && mouseX < timelineEnd && mouseY > individualButtonY + 50) playVideo();
	} else if (!zoomView) {
		reset();
		overZoomButton();
	}
...
function overZoomFamilyButton() {
	if (dist(mouseX, mouseY, zoomFamilyX1, zoomFamilyY) < mapButtonSize) {
		displayFamily = 0;
		familyHighlight(0, 4);
		if (showing) stopVideo(); // have to put inside to not get called evertime and after familyHighlight to set correct video volume
...
function overZoomSpaceButton() {
	if (dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY1) < mapButtonSize) {
		if (showing) stopVideo();
		spaceSelect(0);
	} else if (dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY2) < mapButtonSize) {
		if (showing) stopVideo();
		spaceSelect(1);
	} else if (dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY3) < mapButtonSize) {
		if (showing) stopVideo();
		spaceSelect(2);
	}
}
