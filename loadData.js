function loadDataSmallMultiple() {
    var i;
    var path;
    var block;
    var curPath;
    ...
    for (i = 0; i < individualLength; i++) {
        ...
        if (i == 2 || i == 3 || i == 4 || i == 12) { // for people who did not curate push integer
            ...
            mapCuration.push(curPath);
            mapZoomCuration.push(curPathZoom);
        }
    }
}
...
function loadDataZoom() {
    ...
    for (i = 0; i < individualLength; i++) {
        ...
        mapZoomMovement.push(pathZoom);
        mapZoomTalk.push(blockZoom);
    }
}
...
function loadDataConversation() {
    ...
    for (i = 0; i < conversationLength; i++) {
        conversation = new Conversation(loadStrings(imageFileName + i + "_conversation.txt"), loadImage(imageFileName + i + "_conversationBox.png"), loadImage(imageFileName + i + "_conversationBoxZoom.png"), loadSound("audio/" + i + "_conversationAudio.mp3"));
